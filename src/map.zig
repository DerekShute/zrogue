...
test "put item on map" {
    ...
    var thing = Thing{ .p = Pos.init(25, 25), .tile = .player, .vtable = &vtable };
    try map.setMonster(&thing);
    try expect(try map.getTile(25, 25) == .player);

    try map.removeMonster(25, 25);
    try expect(try map.getTile(25, 25) == .gold);
}
...
test "putting monsters places" {
    var map = try Map.init(std.testing.allocator, 50, 50, 1, 1);
    defer map.deinit();
    var thing = Thing{ .p = Pos.init(10, 10), .tile = .player, .vtable = &vtable };
    var thing2 = Thing{ .p = Pos.init(10, 10), .tile = .player, .vtable = &vtable };
